...
AUPHONIC_API_KEY = "uzGEnKKriAWbEerrXTYWpgOj8VC5AnLU"

# Auphonic presets with full names
AUPHONIC_PRESETS = [
    {
        "id": "q6A8ZcePiR5cy4Aan8czTF",
        "name": "Voice Clean full (maximo) -20 LUFS"
    },
    {
        "id": "LKGxqjqc49E5vrbSSHGcxk",
        "name": "Voice Cleaner (remove breathings verb and noise \"high\") dyn +3db -20 LUFS"
    }
]
...
def process_all_methods(file_path, output_dir):
    """Process audio with all three methods in parallel"""
    print(f"\nProcessing {file_path.name} with all three methods in parallel...")
    
    results = []
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all three processing methods
        auphonic0_future = executor.submit(
            process_with_auphonic, 
            file_path, 
            output_dir, 
        ...
        auphonic1_future = executor.submit(
            process_with_auphonic, 
            file_path, 
            output_dir, 
        ...
        for future in concurrent.futures.as_completed([auphonic0_future, auphonic1_future, elevenlabs_future]):
            try:
                result = future.result()
                if result:
                    results.append(result)
                    print(f"One method complete: {result}")
                else:
                    print("One method failed")
            except Exception as e:
                print(f"Processing error: {str(e)}")
    
    print(f"Parallel processing completed with {len(results)}/{3} successful results")
